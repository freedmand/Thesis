...
Minimum edit distance, or the \textit{Levenshtein distance}, is a distance metric that computes the minimum number of operations needed to transform one sequence into the other using \textit{deletion}, \textit{insertion}, and \textit{substitution}. Deletion entails removing one item from one of the sequences, insertion adding one item to one of the sequences, and substitution replacing an item from one sequence with another item. Each operation entails a cost, and the algorithm finds the minimal cost to transform one sequence into the other in quadratic time.

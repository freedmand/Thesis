...
Because of all its advantages, the Smith-Waterman algorithm will be used to compare chord progressions in this paper. There are downsides to the Smith-Waterman algorithm. It can only be used to extract one optimal local alignment, and the score returned reflects only that local alignment, whereas n-grams comparison does well with multiple like regions of local similarity. There are adjustments to the algorithm to return multiple good alignments, but they prove computationally expensive. This paper focuses on only returning one good alignment. Another difficulty is that of comparing scores from different Smith-Waterman results on different data.

\subsubsection{Normalization}

\[ S_N = \frac{S}{l + N} \] where $S$ is the raw score, $l$ the sum of sequence lengths $n1$ and $n2$, and $N$ a normalization parameter constant \cite{breimer2002learning}



Normalization measures will be discussed later in the paper CITE.

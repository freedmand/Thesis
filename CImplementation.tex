...
static inline int extractBass(int16_t chord) {
  return (chord - 1) / (HARMONY_SIZE * PITCH_SIZE);
}

static inline int extractRoot(int16_t chord) {
  return ((chord - 1) / HARMONY_SIZE) % PITCH_SIZE;
}

static inline int extractHarmony(int16_t chord) {
  return (chord - 1) % HARMONY_SIZE;
...
For quick computation of chord distances based on pitch classes, a 11-bit integer can represent harmony, where the $i$th bit represents whether the interval $i+1$ is included in the chord or not (see figure~\ref{fig:qualitytable}).
